    ...
    if (
        ...
        if (command) {
            try {
                ...
                console.log(
                    `[COMMAND] ${command.name.at(-1)} executed by ${interaction.user.tag} | [${client.user.tag}] in ${interaction.guild.name} (${interaction.guild.id}) | Total Used: ${BStats.all().find((i) => i.ID === command.name.at(-1)).data} times.`,
                );
            } catch (error) {
                console.log(error);
